...
//Nötige Dateien/Header
require './includes/DEF.php';
require './includes/UTILS.php';
require './includes/_top.php';
include "./includes/streets.php";

//Returns postal code, place and house number to a given address.
...
if($_SESSION['Seite'] ==1 || $_SESSION['Seite'] ==2){
	...
	if($_SESSION['Seite'] ==1 ||$_SESSION['Seite'] ==3){
		...
		if ($stop == '1') {
			echo '<h3><red>Eine andere Tat ist bereits unter diesem Namen veröffentlicht.</red></h3>';
		}
		if ($stop == '2') {
			echo '<h3><red>Bitte einen neuen Namen eingeben.</red></h3><br>';
		}
		if ($stop == '3') {
			echo '<h3><red>Wähle den Typ deiner Tat aus.</red></h3><br>';
		}
		//le buttons
		echo'
		<br><br>
...
if($_SESSION['Seite'] ==2 || $_SESSION['Seite']==3){
	if (isset($_FILES['pictures'])) {
		$_SESSION['tat_pictures'] = $_FILES['pictures'];
	}
	if($_SESSION['Seite']==2){
		echo'
			...
			<br>
			<br>';
		if (isset($_SESSION['tat_pictures']['name']) && $_SESSION['tat_pictures']['name'] == '') {
			$_SESSION['tat_pictures'] = '';
		}
...
if(($_SESSION['Seite'] ==3 || $_SESSION['Seite'] ==4)){        
	if($_SESSION['Seite'] ==4){
	    if (isset($_POST['description'])) {
			$_SESSION['tat_description'] = $_POST['description'];
		}
	...
	if($_SESSION['Seite'] ==3){        
		...
		if ($stop == 1) {
			echo '<h3><red>Bitte beschreibe deine Tat.</red></h3>';
		}
		echo '
		<br><br>
		<div class="center block deeds_create">
...
if(($_SESSION['Seite'] ==4 ||$_SESSION['Seite'] ==5)){
	...
	if($_SESSION['Seite'] ==4){
		...
		if ($start_dh === false) {
			$start_dh = (new DateHandler())->setHours(8)->setMinutes(0);
		}
		...
		if ($end_dh === false) {
			$end_dh = (new DateHandler())->setHours(8)->setMinutes(0);
		}
...
if($_SESSION['Seite'] ==5){        
	...
	// Added additional info text if house number wasn't found.
