...
if(isset($_FILES['picture'])){
	...
	if($data==NULL){
		...
		//Bei Bedarf aendern
		$picture_size = 600*1024; 
		$kb = $picture_size/1024;
		//falls es mehr Bildformate geben sollte, bitte ergänzen
		$picture_format = array('jpeg','jpg', 'gif','png');
		$file_extension = strtolower(pathinfo($_FILES['picture']['name'], PATHINFO_EXTENSION));
		// Fehler überprüfen
		if(!in_array($file_extension, $picture_format)) {
		//zündet auch wenn kein Bild angegeben wurde...wird noch gefixt
		//echo'<h2>Bitte ein Bild hochladen.</h2>';
		}else if($_FILES['picture']['size'] > $picture_size) {
...
//Tat-Objekt holen -> jetzt damit es aktuell ist
$tat = DBFunctions::db_getGuteTat($idGuteTat);	
