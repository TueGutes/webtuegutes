...
if (!isset($_GET['page'])) {
	$_GET['page'] = 1 && $placeholder = "Alle";
}
if (!isset($_POST['status'])) {
	$_POST['status'] = 'alle';
	if(!isset($_REQUEST['number_of_entries'])){
		$_REQUEST['number_of_entries'] = 10;
	}
	$first=TRUE;
} 
else {
...
?>
	<h5>
		...
		&nbsp;
		Taten pro Seite:
		<select name="number_of_entries" size="1" onchange="this.form.submit()">
			<option value="5" <?php echo (@$_REQUEST['number_of_entries']==5)?'selected':'' ?> >5</option>        <?/*hier wird der status abgefragt */?>
			<option value="10" <?php echo (@$_REQUEST['number_of_entries']==10)?'selected':'' ?> >10</option>     <?/*hier wird der status abgefragt */?>
			<option value="15" <?php echo (@$_REQUEST['number_of_entries']==15)?'selected':'' ?> >15</option>	 <?/*hier wird der status abgefragt */?>
			<option value="20" <?php echo (@$_REQUEST['number_of_entries']==20)?'selected':'' ?> >20</option>      <?/*hier wird der status abgefragt */?>
			<option value="50" <?php echo (@$_REQUEST['number_of_entries']==50)?'selected':'' ?> >50</option>     <?/*hier wird der status abgefragt */?>
			<option value="100" <?php echo (@$_REQUEST['number_of_entries']==100)?'selected':'' ?> >100</option>	 <?/*hier wird der status abgefragt */?>
		</select>
		<noscript><input type="submit" name="button" value="Aktualisieren"/></noscript>
	</h5>
</form>
...
$tatenProSeite=$_REQUEST['number_of_entries'];
$all = !(isset($_GET['user']) && DBFunctions::db_getIdUserByUsername($_GET['user']!=-1));

if ($all) {
	$merge = $admin && $placeholder == 'alle';
	if ($merge) {
		$arr2 = DBFunctions::db_getGuteTatenForList($tatenProSeite * ($_GET['page'] - 1), $tatenProSeite, 'nichtFreigegeben');
	}
...
if ($letzteseite * $tatenProSeite < $anzahlAllerTaten) {
	$letzteseite++;
}
...
function zahlenausgabe($von,$bis, $letzteseite, $all){//schreibt die seiten zahlen
	for($i=$von;$i<=$bis;$i++){
		...
		if ($i > 0 && $i <= $letzteseite) {
			echo '<a href="./deeds?page=' . $i . (!$all ? '&user=' . $_GET['user'] : '') . '&number_of_entries=' . ($_REQUEST['number_of_entries']) . '">&nbsp' . $i . '&nbsp</a>';
		}
	}
}
...
if($seitenanzahl>$maxSeitenLinks){
	if($aktuelleSeite<($maxSeitenLinks/2)+1){
		...
		if ($letzteseite > $maxSeitenLinks) {
			printf('<a href="./deeds?page=' . $letzteseite . (!$all ? '&user=' . $_GET['user'] : '') . '&number_of_entries=' . ($_REQUEST['number_of_entries']) . '"> ... ' . $letzteseite . '</a>');
		}
	} else if ($aktuelleSeite > $letzteseite- ($maxSeitenLinks/2)-1) {
		if ($letzteseite > $maxSeitenLinks) {
			printf("<a href='./deeds" . (!$all ? '?user=' . $_GET['user'] : '') . '&number_of_entries=' . ($_REQUEST['number_of_entries']) . "'>" . '1 ... ' . "</a>");
		}// der link geht jetzt
		...
		printf ("<a href='./deeds" . (!$all?'?user='.$_GET['user']:'') .'&number_of_entries='.($_REQUEST['number_of_entries']). "'>". '1 ... ' ."</a>");// der link geht jetzt
...
if ($vor) {
	echo '<a href="./deeds?page=' . ($_GET['page'] - 1) . '&number_of_entries=' . ($_REQUEST['number_of_entries']) . '"><input type="button" value="Zurück"></a>';
}
if ($vor && $nach) {
	echo '&nbsp';
}
if ($nach) {
	echo '<a href="./deeds?page=' . ($_GET['page'] + 1) . '&number_of_entries=' . ($_REQUEST['number_of_entries']) . '"><input type="button" value="Weiter"></a>';
}
?>
