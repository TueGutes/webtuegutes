...
function zahlenausgabe($von,$bis, $letzteseite, $all){//schreibt die seiten zahlen
	for($i=$von;$i<=$bis;$i++){
		...
		if ($i > 0 && $i <= $letzteseite) {
			echo '<a href="./admin?page=deeds&p=' . $i . (!$all ? '&user=' . $_GET['user'] : '') . '&number_of_entries=' . ($_REQUEST['number_of_entries']) . '">&nbsp' . $i . '&nbsp</a>';
		}
	}
}
...
if($seitenanzahl>$maxSeitenLinks){
	if($aktuelleSeite<($maxSeitenLinks/2)+1){
		...
		if ($letzteseite > $maxSeitenLinks) {
			printf('<a href="./admin?page=deeds&p=' . $letzteseite . (!$all ? '&user=' . $_GET['user'] : '') . '&number_of_entries=' . ($_REQUEST['number_of_entries']) . '"> ... ' . $letzteseite . '</a>');
		}
	} else if ($aktuelleSeite > $letzteseite- ($maxSeitenLinks/2)-1) {
		if ($letzteseite > $maxSeitenLinks) {
			printf("<a href='./admin?page=deeds" . (!$all ? '?user=' . $_GET['user'] : '') . '&number_of_entries=' . ($_REQUEST['number_of_entries']) . "'>" . '1 ... ' . "</a>");
		}// der link geht jetzt
		...
		printf ("<a href='./admin?page=deeds" . (!$all?'?user='.$_GET['user']:'') .'&number_of_entries='.($_REQUEST['number_of_entries']). "'>". '1 ... ' ."</a>");// der link geht jetzt
...
<?php
	...
	if ($vor) {
	echo '<a href="./admin?page=deeds&p=' . ($_GET['p'] - 1) . '&number_of_entries=' . ($_REQUEST['number_of_entries']) . '"><input type="button" value="Zurück"></a>';
}
if ($vor && $nach) {
	echo '&nbsp';
}
if ($nach) {
	echo '<a href="./admin?page=deeds&p=' . ($_GET['p'] + 1) . '&number_of_entries=' . ($_REQUEST['number_of_entries']) . '"><input type="button" value="Weiter"></a>';
}
?>
